...
class ChunkServer:
    ...
    def send_heartbeats(self):
        """Send periodic heartbeat messages to all MasterServers."""
        while True:
            ...
            for master_host, master_port in MASTER_SERVERS:
                try:
                    ...
                    logging.info("Heartbeat sent to MasterServer at %s:%d", master_host, master_port)
                except Exception as e:
                    logging.error("Failed to send heartbeat to MasterServer at %s:%d - %s", master_host, master_port, e)

    def calculate_checksum(self, data):
        """Calculate the checksum of data for integrity checks."""
        return hashlib.sha256(data).hexdigest()

    def store_chunk(self, chunk_id, filename, data, checksum, is_replica=False):
    ...
    def handle_request(self, client, address):
        ...
        try:
            ...
            if command == 'store':
                ...
                data = request['data']
                checksum = request['checksum']
                response = self.store_chunk(chunk_id, filename, data, checksum, is_replica=True)
                client.send(pickle.dumps(response))
    ...
    def send_chunk(self, chunk_id, filename):
        ...
        try:
            base_filename = os.path.basename(filename)
            chunk_name = f"{base_filename}_chunk_{chunk_id}.txt"
            chunk_path = os.path.join(self.base_directory, chunk_name)
...
if __name__ == "__main__":
    try:
        ...
        base_directory = os.path.join(os.getcwd(), f"chunk_server_{port_num}")
